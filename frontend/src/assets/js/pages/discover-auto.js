  ...
  <header class="sticky top-0 z-20 bg-background-dark/95 backdrop-blur-md border-b border-white/5 p-4">
    <div class="flex items-center gap-3">
      ...
      <div class="flex flex-col">
        <h2 class="text-xl font-bold text-white">Auto-Discovery</h2>
      </div>
    </div>
  </header>
...
export function init() {
  ...
  async function discoverServers() {
    ...
    discoveryTimeout = setTimeout(() => stopDiscovery(), 15000);
    try {
      ...
      <div class="p-4 flex items-center gap-4">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
          ...
          <div class="flex items-center gap-2 mt-0.5">
            ...
            ${server.AnonymousAllowed ? '<span class="text-[9px] bg-green-500/10 text-green-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Public</span>' : '<span class="text-[9px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Private</span>'}
          </div>
        </div>
  ...
  async function connectWithCredentials(host, port, user, pass, isAnon, name) {
    ...
    try {
      ...
      if (response.ok || text.includes("already connected")) {
        ...
        if (globalThis.Components?.showToast)
          globalThis.Components.showToast(`Connection failed: ${text}`, "error");
      }
    } catch {
      if (globalThis.Components?.showToast)
        globalThis.Components.showToast("Network error or server unreachable", "error");
    }
  }
  ...
  if (submitBtn) submitBtn.onclick = async () => {
    ...
    if (!user || !pass) {
      if (globalThis.Components?.showToast)
        globalThis.Components.showToast("Please enter both username and password", "error");
      return;
    }
