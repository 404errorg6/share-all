...
async function handleRoute() {
  ...
  try {
    ...
    if (
      ...
      typeof globalThis.Components.Sidebar.highlight === "function"
    ) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const response = await fetch("/api/p2p/invite/pending");
    if (response.ok) {
      const data = await response.json();
      if (data && data.invite) {
        window.location.hash = `#/discover?invite=${encodeURIComponent(data.invite)}`;
        return;
      }
    }
  } catch (err) {
    console.warn("Failed to load pending P2P invite:", err);
  }
