...
globalThis.Components.Preview = {
  ...
  async show(path, name, isRemote = true) {
    ...
    if (subtitleLabel)
      subtitleLabel.innerText = isRemote ? "Remote Preview" : "Local Preview";
    ...
    const api = isRemote
      ? "/api/ftp/server/get-file"
      : "/api/ftp/client/get-file";
    const paramName = isRemote ? "remote_path" : "local_path";
    let url = `${api}?${paramName}=${encodeURIComponent(path)}`;
    ...
    try {
      if (isRemote && isP2PTransport()) {
        const mimeType = this.supportedFormats.image.includes(ext)
          ? `image/${ext === "jpg" ? "jpeg" : ext}`
          : this.supportedFormats.video.includes(ext)
            ? `video/${ext}`
            : this.supportedFormats.pdf.includes(ext)
              ? "application/pdf"
              : "text/plain";
        url = await P2P_API.getPreviewUrl(path, mimeType);
      }
